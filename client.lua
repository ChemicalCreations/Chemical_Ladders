...
function getLineUnitScale3D(sx, sy, sz, ex, ey, ez, m)
	local m = m or 1
	local sx, sy, sz, ex, ey, ez = tonumber(sx) or 0, tonumber(sy) or 0, tonumber(sz) or 0, tonumber(ex) or 0,
		tonumber(ey) or 0, tonumber(ez) or 0
...
if testing then
	bindKey("r", "down", function()
		...
		local deci = 1000
		x, y, z, r = math.floor(x * deci) / deci, math.floor(y * deci) / deci, math.floor(z * deci) / deci,
			math.floor(r * deci) / deci
		texter = { "{sx=", x, " , sy=", y, ", sz=", z, ", tx=", x, ", ty=", y, ", tz=", z, ", rx=0, ry=0, rz=", r,
			", d=1}," }
		local text = ""
		for i = 1, #texter do
			local t = texter[i]
...
bindKey("enter_exit", "down", function()
	...
	if climbers[ped] then
		...
		local block, anim, time, loop, updatePosition, interuptable, fLF, blendTime = "BSKTBALL", "BBall_idle2_O", 0,
			false, false, false, true, 600
		setPedAnimation(ped, block, anim, time, loop, updatePosition, interuptable, fLF, blendTime)
		data.state = "fall"
		data.prog = 0
		data.dir = nil
		data.next = nil
		triggerServerEvent("onPlayerReportLadderClimbingState", ped, data.surface, data.climb, data.position, data.state,
			data.prog, data.dir, data.dir_next)
		return
	end
	...
	for surface, surfaceData in pairs(climbs) do
		for climb, l in pairs(surfaceData) do
			local sx, sy, sz, tx, ty, tz, rx, ry, rz, d, r = l.sx, l.sy, l.sz, l.tx, l.ty, l.tz, l.rx, l.ry, l.rz, l.d,
				l.r
		...
		while prog ~= 0 do
			...
			if data.dir or not state.anim_hold then
				...
				if dir < 0 and data.prog <= state.anim_start then
					data.prog, prog, data.next = state.anim_start, state.anim_start - data.prog,
						data.next or state.climb_down
